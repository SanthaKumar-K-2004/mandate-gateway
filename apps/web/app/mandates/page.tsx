    ...
    <div className="min-h-screen bg-[#090d16] text-slate-100 font-sans pb-16">
      ...
      <header className="border-b border-slate-800/80 px-8 py-5 flex justify-between items-center bg-[#0d1322]">
        <div className="flex items-center gap-3">
          <Link href="/" className="w-9 h-9 bg-gradient-to-br from-amber-500 to-orange-600 rounded-xl flex items-center justify-center font-black text-lg text-white shadow-lg shadow-amber-500/20">
            M
          </Link>
          <div>
            <h1 className="text-lg font-extrabold text-white m-0">BUYER MANDATES</h1>
            <p className="text-xs text-slate-400 m-0">Active authorization mandates, spend caps & one-click revocation</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => setShowCreate((v) => !v)}
            className="text-xs font-bold bg-amber-600 hover:bg-amber-500 text-white px-4 py-2 rounded-lg border border-amber-500/40 transition"
          >
            + New Mandate
          </button>
