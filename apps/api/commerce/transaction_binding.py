...
class CommerceTransactionBinding:
    """1:1 Binding between RAZORPAY Payment Transaction and Merchant Order."""

    binding_id: str
    razorpay_transaction_id: str
    ...
    def to_dict(self) -> Dict[str, str | int]:
        return {
            ...
            "razorpay_transaction_id": self.razorpay_transaction_id,
    ...
    @property
    def razerpay_transaction_id(self) -> str:
        """Backwards compatibility alias for razorpay_transaction_id."""
        return self.razorpay_transaction_id
...
class CommerceTransactionBindingManager:
    ...
    def bind_transaction_to_order(
        ...
        """
        if razorpay_transaction_id in self._tx_map:
            existing = self._tx_map[razorpay_transaction_id]
            if existing.merchant_order_id != merchant_order_id:
                raise TransactionBindingError(
                    f"Transaction '{razorpay_transaction_id}' is already bound to "
        ...
        if merchant_order_id in self._order_map:
            ...
            if existing.razorpay_transaction_id != razorpay_transaction_id:
                raise TransactionBindingError(
                    f"Merchant order '{merchant_order_id}' is already bound to "
                    f"transaction '{existing.razorpay_transaction_id}'."
                )
